...
@Controller
// [수정] 클래스 레벨의 공통 경로를 /postManagement로 변경
...
public class PostManagementController {
    ...
    @GetMapping
    public String postList(@ModelAttribute AdminPost searchCriteria, Model model) {
        ...
        int totalCount = postManagementService.getPostCount(searchCriteria);
        Pagination pagination = new Pagination(totalCount, searchCriteria);
        searchCriteria.setOffset(pagination.getLimitStart());
        
        // 데이터 조회
        List<AdminPost> postList = postManagementService.getPostList(searchCriteria);
    ...
    public ResponseEntity<?> updatePostsStatus(@RequestBody Map<String, Object> payload,
        ...
        try {
            if ("hide".equals(action)) {
                postManagementService.hidePosts(postSns);
            } else if ("restore".equals(action)) {
                postManagementService.restorePosts(postSns);
            } else {
