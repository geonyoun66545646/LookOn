...
public class PostRestController {
	...
	@GetMapping("/list")
	public ResponseEntity<Map<String, Object>> selectPostList(@RequestParam(required = false) String bbsClsfCd,
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(Map.of("postList", Collections.emptyList(), "hasNext", false));
		}
	}

	@PostMapping(value = "/write", consumes = { "multipart/form-data" })
	public ResponseEntity<Map<String, Object>> insertPost(@RequestParam("bbsClsfCd") String bbsClsfCd,
			@RequestParam("pstTtl") String pstTtl, @RequestParam("pstCn") String pstCn,
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.CREATED)
					.body(Map.of("result", "success", "message", "게시글이 성공적으로 작성되었습니다."));
		} catch (Exception e) {
			log.error("게시글 작성 중 오류 발생: {}", e.getMessage(), e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(Map.of("result", "fail", "message", "게시글 작성 중 오류가 발생했습니다."));
		}
	}

	@PostMapping(value = "/update/{pstSn}", consumes = { "multipart/form-data" })
	public ResponseEntity<Map<String, Object>> updatePost(@PathVariable("pstSn") String pstSn,
			@RequestParam("bbsClsfCd") String bbsClsfCd, @RequestParam("pstTtl") String pstTtl,
		...
		try {
			...
			if (isSuccess) {
				...
				return ResponseEntity.status(HttpStatus.FORBIDDEN)
						.body(Map.of("result", "fail", "message", "게시글을 수정할 권한이 없습니다."));
			}
		} catch (Exception e) {
			log.error("게시글 수정 중 오류 발생: pstSn={}, {}", pstSn, e.getMessage(), e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(Map.of("result", "fail", "message", "게시글 수정 중 오류가 발생했습니다."));
		}
	}
	...
	public ResponseEntity<Map<String, Object>> deletePost(@PathVariable String pstSn, HttpSession session) {
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(Map.of("result", "fail", "message", "게시글 삭제 중 오류 발생"));
		}
	}

	@PostMapping("/toggle-like")
	public ResponseEntity<Map<String, Object>> addPostLike(@RequestBody Map<String, String> payload,
			HttpSession session) {
		LoginUser loginUser = (LoginUser) session.getAttribute("loginUser");
		if (loginUser == null) {
			return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
	...
	public ResponseEntity<Map<String, Object>> addComment(@RequestBody PostComment comment, HttpSession session) {
		LoginUser loginUser = (LoginUser) session.getAttribute("loginUser");
		if (loginUser == null) {
			return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
		}
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(Map.of("result", "fail", "message", "댓글 등록 중 오류 발생"));
		}
	}

	@PutMapping("/comments/{pstCmntSn}")
	public ResponseEntity<Map<String, Object>> updateComment(@PathVariable String pstCmntSn,
			@RequestBody PostComment comment, HttpSession session) {
		LoginUser loginUser = (LoginUser) session.getAttribute("loginUser");
		if (loginUser == null) {
			return ResponseEntity.status(HttpStatus.UNAUTHORIZED).build();
		}
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(Map.of("result", "fail", "message", "댓글 수정 중 오류 발생"));
		}
	}
	...
	public ResponseEntity<Map<String, Object>> deleteComment(@PathVariable String pstCmntSn, HttpSession session) {
		...
		try {
			...
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
					.body(Map.of("result", "fail", "message", "댓글 삭제 중 오류 발생"));
		}
	}
}