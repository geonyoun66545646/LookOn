...
public class SearchServiceImpl implements SearchService {
	...
	/**
	 * ▼▼▼ 기존에 있던 코드 (수정 없음) ▼▼▼ '전체' 탭의 서버 렌더링을 위한 메서드. 각 항목별로 5개의 미리보기 데이터를 가져옵니다.
	 */
	@Override
	public Search searchAll(String keyword, String userNo, String ipAddress) { // (수정) ipAddress 파라미터 추가

		// 1. 먼저 모든 검색 결과를 가져옵니다.
		...
		// 2. 검색된 결과의 총 개수를 계산합니다.
		long totalResultCount = products.size() + posts.size() + users.size();

		// 3. 로그인한 사용자의 경우, 모든 정보를 취합하여 검색 기록을 저장합니다.
