$(document).ready(function() {
	...
	function createExistingImagePreviews() {
		...
		productData.productImages.forEach(img => {
			...
			if ($previewContainer.find('p').length) $previewContainer.empty();
			...
			// 기존 이미지 정보를 hidden input으로 저장 (삭제 구분을 위해 이미지 타입까지 명시)
			$('#productRegistrationForm').append(
				`<input type="hidden" class="existing-image-path" name="existingImagePaths" value="${img.imgFilePathNm}" data-image-no="${img.imgNo}" data-image-type="${img.imgType}">`
			);
		});

		// ⭐ 추가: 미리보기 영역을 sortable로 활성화 (드래그앤드롭)
		$('#thumbnailImagePreview, #mainImagePreview, #detailImagePreview').sortable({
			items: '.image-upload-item',
			update: function(event, ui) {
				console.log('이미지 순서 변경됨');
				// 필요시 서버 전송용 input 업데이트 가능
			}
		});
	}
	...
	$(document).on('click', '.image-upload-item .remove-btn', function() {
		...
		const inputId = $(this).data('target-input');
