$(() => {
    ...
    const createFeedDetailHtml = (feed) => {
		...
		// ▼▼▼ [디버깅 코드 추가] ▼▼▼
        // isMyPost를 결정하는 두 변수의 값을 콘솔에 직접 출력하여 확인합니다.
        console.log("--- 팔로우 버튼 생성 조건 확인 ---");
        console.log("로그인한 사용자 (loginUserNo):", loginUserNo, "(타입:", typeof loginUserNo, ")");
        console.log("게시물 작성자 (feed.wrtrUserNo):", feed.wrtrUserNo, "(타입:", typeof feed.wrtrUserNo, ")");
    ...
    container
		...
		/**
	     * [신규 추가]
	     * 화면에 있는 모든 피드 게시물에 대해 팔로우 버튼을 초기화하는 함수입니다.
	     * 무한 스크롤로 새로운 피드가 추가될 때도 호출할 수 있습니다.
	     */
	    function initializeAllFollowButtons() {
	        // 아직 팔로우 버튼이 생성되지 않은(.follow-initialized 클래스가 없는) 게시물 헤더를 모두 찾습니다.
	...
	// 무한 스크롤로 피드가 추가된 후에도 팔로우 버튼을 생성하도록 수정합니다.
   const originalDoneCallback = $.ajaxSettings.done; // 기존의 done 콜백을 잠시 저장
   $.ajaxSetup({
       done: function(data, textStatus, jqXHR) {
           // 무한 스크롤 응답(nextFeedList)을 받은 경우에만 실행
           if (jqXHR.responseJSON && Array.isArray(jqXHR.responseJSON)) {
               // 새로운 피드가 화면에 추가된 후, 팔로우 버튼을 다시 초기화합니다.
               setTimeout(initializeAllFollowButtons, 100); 
           }
           if(originalDoneCallback) originalDoneCallback.apply(this, arguments); // 원래의 done 콜백 실행
       }
   });
